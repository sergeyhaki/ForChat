...
Этот код реализует функцию дискретного преобразования Фурье (DFT) для входной последовательности `x` и записывает результат в массив `X`. Входная последовательность заполняется значениями от 0 до 15, а результаты DFT выводятся на экран.
