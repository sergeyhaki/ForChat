ке Си, который использует целочисленную арифметику с 16-битными целыми числами:```c
...
DFTvoid idft(short *real, short *imag, short *x) { 
 for (int n = 0; n < N; n++) {  
  ...
  x[n] /= N; // Нормировка  }}int main() {  dft(x, real, imag);  idft(real, imag, x);  for (int i = 0; i < N; i++) {    printf("%d\n", x[i]);  }  return 0;}```Здесь `short` обозначает 16-битные целые числа, используемые для представления входных данных, действительной и мнимой частей спектра и 
